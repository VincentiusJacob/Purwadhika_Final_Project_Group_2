...
from .job_information_agent import get_job_info_agent

job_info_agent = get_job_info_agent()

supervisor_agent = create_supervisor(
    agents=[job_info_agent],
    model=ChatOpenAI(model="gpt-4o-mini"),
    prompt=(
        "You manage a job  assistant, assign work to them."
    )
).compile()
