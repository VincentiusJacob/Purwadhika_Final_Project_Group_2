...
def main():
    ...
    def create_rag_text(row):
        salary_info = "Negotiable"
        if row['min_salary'] > 0:
            salary_info = f"IDR {row['min_salary']:,} - {row['max_salary']:,}"
            
        return (
            f"Job Title: {row['job_title']}\n"
            f"Company: {row['company_name']}\n"
            f"Location: {row['clean_location']} ({row['work_style']})\n"
            f"Salary Range: {salary_info}\n"
            f"Type: {row['work_type']}\n"
            f"Description:\n{row['job_description']}"
        )

    df['rag_content'] = df.apply(create_rag_text, axis=1)
    
    # Kolom yang diperlukan untuk embedding.
    rag_data = df[['rag_content', 'job_title', 'company_name', 'work_style']].to_dict(orient='records')
    
    with open(RAG_OUTPUT, 'w', encoding='utf-8') as f:
        json.dump(rag_data, f, indent=2, ensure_ascii=False)
        
    print("Data RAG berhasil diexport!")
